...
class ImportExportUtils:
    ...
    def export_reviews_json(reviews: list) -> dict:
        """Reviews als JSON exportieren (Legacy-Format)"""
        export_data = {
    ...
    def make_json_serializable(data):
        """Macht komplexe Datenstrukturen JSON-serialisierbar"""
        if isinstance(data, dict):
            return {key: ImportExportUtils.make_json_serializable(value) for key, value in data.items()}
        elif isinstance(data, list):
            return [ImportExportUtils.make_json_serializable(item) for item in data]
